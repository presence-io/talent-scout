...
├── SKILLS.md                 # 全局积累的运行经验（OpenClaw 管理）
├── scripts/
...
│   │   │   ├── review.ts     # 评估结果审查与 SKILLS 更新
│   │   │   └── SKILLS.md     # AI 评估模块积累的经验
│   │   ├── __tests__/
│   │   └── package.json
│   │
...
│   ├── raw/                  # 原始 API 响应，按 YYYY-MM-DD_HHmmss/ 分目录
│   ├── processed/            # 处理后的候选人数据
│   ├── evaluated/            # AI 评估结果
│   └── latest -> ...         # 符号链接指向最近一次运行
│
├── cache/                    # API 响应缓存（.gitignore）
│   ├── github/               # GitHub API 响应缓存
│   └── rankings/             # 外部排行榜快照缓存
│
...
- `output/` 是**运行产出**：每次运行都创建新的时间戳目录，保留历史记录，永远不覆盖
- `cache/` 是**请求缓存**：跨运行复用，按 key（API endpoint + params hash）去重，有 TTL 策略
- `output/latest` 是一个符号链接，始终指向最近一次运行的目录，方便 Dashboard 和后续步骤直接读取
...
```
  ...
  Rankings ────────>│  rankings                    │──> output/raw/{timestamp}/
  ...
  Follower graph ──>│  follower-graph              │
                    ...
                    │       data-processor          │
                    │                              │
  output/raw/ ─────>│  merge → identity → enrich   │──> output/processed/{timestamp}/
  cache/ ──────────>│  → scoring                   │
  seeds/ ──────────>│                              │
                    └──────────┬───────────────────┘
                               │
                    ...
                    │       ai-evaluator            │
                    │                              │
  output/processed/ >│  evaluate → review           │──> output/evaluated/{timestamp}/
  SKILLS.md ───────>│  → SKILLS update             │
  openclaw agent ──>│                              │
                    └──────────┬───────────────────┘
                               │
                               ▼
                    ...
                    │       dashboard               │
                    │                              │
  output/evaluated/ >│  展示 + 筛选 + 排序 + 标注   │──> 浏览器
  output/latest ───>│  修改反馈写回 JSON            │
                    └──────────────────────────────┘
```
