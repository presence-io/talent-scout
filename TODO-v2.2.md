...
- [ ] **1.2** 实现 `packages/skills/src/` 核心模块
...
- [ ] **1.3** 更新 `pnpm-workspace.yaml` 包含 `packages/skills`。

- [ ] **1.4** 更新 `.github/workflows/publish.yml`
...
- [ ] **1.5** 添加 skills 包测试，确保 `pnpm check` 通过。

## Phase 2: P1 — 建立共享查询层

**目标**：让 Dashboard 和 `@talent-scout/skills` 复用同一套查询接口，不再各自直接解析 JSON 文件。

- [ ] **2.1** 在 `@talent-scout/ai-evaluator` 中暴露查询 API
...
- [ ] **2.2** 在 `@talent-scout/data-processor` 中暴露查询 API
...
- [ ] **2.3** 在 `@talent-scout/data-collector` 中暴露查询 API
  - 导出 `loadRawSignals(rawDir): Promise<Record<string, Signal[]>>`。

- [ ] **2.4** 重构 Dashboard 使用共享查询接口
...
- [ ] **5.3** 更新测试，确保 `pnpm check` 通过。

## Phase 6: P2 — 清理旧版输出产物与杂项

**目标**：清理旧格式文件，对齐仓库状态与代码逻辑。

- [ ] **6.1** 删除 `output/` 下的旧版 `step*` 文件
