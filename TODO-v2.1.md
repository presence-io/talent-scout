...
- [ ] **2.4** 更新 ai-evaluator CLI 入口
  - `packages/ai-evaluator/src/index.ts` 使用新的输入/输出目录。

- [ ] **2.5** 更新 Dashboard 文件读取
...
- [ ] **6.4** 更新测试，确保 `pnpm check` 通过。

## Phase 7: P2 — 清理与配置对齐

**目标**：清理占位代码，对齐配置与设计。

- [ ] **7.1** 清理 `packages/shared/src/index.ts` 的占位注释。

- [ ] **7.2** 确保 `pnpm check` 通过。

## Phase 8: 包发布配置

**目标**：为 npm 公开发布做准备。

- [ ] **8.1** 移除各包 `package.json` 中的 `"private": true`
  - 添加 `publishConfig: { "access": "public" }`。
  - 添加 `files` 白名单（`["dist", "src"]`）。
  - 添加 `main` / `types` 指向编译产出。
  - Dashboard 保留 `"private": true`（不发布）。

- [ ] **8.2** 创建 `.github/workflows/publish.yml`
  - 监听 `v*` tag push。
  - 提取版本号，批量更新各包版本。
  - 执行 install → build → test → publish。
  - 按依赖顺序 `pnpm publish --access public --no-git-checks`。

- [ ] **8.3** 确保 `pnpm check` 通过。
